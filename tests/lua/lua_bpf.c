...
static void lua_prepare_body(void) {
    ...
    lua_runner_control.input.address = lua_input_buf;
...
static void lua_run_body(void) {
    ...
    if (!state) {
        lua_capsule_error(0, "cannot create Lua state", 23);
        capsule_exit(1);
    }
    luaL_openlibs(state);
    ...
    lua_pop(state, 1);
    int status = luaL_loadbuffer(state, lua_script_buf, (unsigned long)lua_runner_control.script.size, "bpf.lua");
    if (!status) {
        status = lua_pcall(state, 0, 0, 0);
    }
    if (status) {
        lua_capsule_report_error(state);
        lua_close(state);
        capsule_exit(1);
    }
    lua_close(state);
