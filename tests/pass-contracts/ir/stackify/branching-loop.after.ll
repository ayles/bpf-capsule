...
branching_loop.prologue:                          ; preds = %unit.test.left
  ...
  br i1 %7, label %branching_loop.prologue.overflow, label %entry
...
latch.chunk.guard:                                ; preds = %latch
  ...
  %8 = icmp ult i32 %bpf.loop.chunk.next, 8
  br i1 %8, label %latch.chunk.continue, label %latch.chunk.boundary

latch:                                            ; preds = %add.one, %add.two
  %sum.next = phi i32 [ %sum.one, %add.one ], [ %sum.two, %add.two ]
  %next = add i32 %index, 1
  %9 = getelementptr i8, ptr %frame.addr, i64 24
  %count = load i32, ptr %9, align 4
...
exit:                                             ; preds = %latch
  ...
  %10 = getelementptr i8, ptr %frame.addr, i64 8
  %return.pc = load i32, ptr %10, align 4
  %11 = getelementptr i8, ptr %frame.addr, i64 0
  %saved.fp = load i64, ptr %11, align 8
...
unit.test.left:                                   ; preds = %unit.dispatch4
  br label %branching_loop.prologue
