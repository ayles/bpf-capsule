...
define i32 @root(i64 %count, i32 %value) !bpf.capsule !0 {
entry:
  %frame_alignment = alloca i8, align 32
  store i8 9, ptr %frame_alignment, align 32
  %vla = alloca i8, i64 %count, align 32
  store i8 5, ptr %vla, align 1
  %first = call i32 @ordinary_helper(i32 %value)
  %second = call i32 @ordinary_helper(i32 %first)
  %byte = load i8, ptr %vla, align 1
  %extended = zext i8 %byte to i32
  %result = add i32 %second, %extended
  ret i32 %result
}

define i32 @start(i32 %fiber) section "syscall" !bpf.native !0 {
entry:
  %result = call i32 @root(i64 8, i32 7) [ "bpf.capsule.call"(i32 %fiber) ]
  ret i32 %result
}
