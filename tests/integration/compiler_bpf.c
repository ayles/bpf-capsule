...
// the soft-float intrinsic fallback, not of a C library shim.
...
#if BPF_CAPSULE_TARGET_KERNEL >= 6009
...
int compiler_fp_intrinsic_run(void) {
    // The guard publishes the encoded exit word and yields zero, so the body
    // still runs to completion; the framework code surfaces on the result.
    struct capsule_result result = capsule_call_void(compiler_fp_intrinsic_body);
...
static void compiler_fiber_body(unsigned int value, unsigned int other) {
    ...
    // The integration target deliberately uses a 256-step drive, so this
