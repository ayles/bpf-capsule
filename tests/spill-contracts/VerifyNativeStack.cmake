...
execute_process(
    COMMAND
        ...
        -bpf-unified-spill-limit=0 -bpf-stack-size=262144 -verify-machineinstrs -o "${WORK}/native-zero.o"
        "${SOURCE_DIR}/native-zero.ll"
...
execute_process(
    COMMAND "${LLVM_READELF}" --relocations "${WORK}/native-zero.o"
    RESULT_VARIABLE result
...
execute_process(
    COMMAND
        "${BPF_CAPSULE_LD}" --passes=no-op-module -mcpu=${TARGET_CPU} -bpf-unified-spill-pipeline
        "-bpf-unified-spill-limit=${native_limit}" -bpf-stack-size=262144 -verify-machineinstrs -o
        "${WORK}/oversized-native.o" "${SOURCE_DIR}/oversized-native.ll"
    RESULT_VARIABLE result
    OUTPUT_VARIABLE output
    ERROR_VARIABLE error
)
...
if(
    NOT log MATCHES "native function oversized_native exceeds the 512-byte BPF stack"
    AND NOT log MATCHES "native BPF call path uses [0-9]+ stack bytes; kernel limit is 512: oversized_native"
)
