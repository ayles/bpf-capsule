...
//   capsule_call_ctx(ctx, &out, root, args...)
//                                      start with a borrowed verifier context
//   capsule_call_void_ctx(ctx, root, args...)
//   capsule_continue(&out, cont)        resume a PENDING/YIELD computation
//   capsule_continue_void(cont)         same, for a void root
//   capsule_continue_ctx(ctx, &out, cont)
//                                      resume while lending a live context
//   capsule_continue_void_ctx(ctx, cont)
...
// that must be passed to exactly one later matching capsule_continue variant
// (resumes immediately after this call) or capsule_reset (cancels and releases
// the fiber). Request and response data are an application protocol; the
// Capsule runtime only reports the suspension.
extern void __bpf_capsule_yield(void);
static __attribute__((always_inline)) inline void capsule_yield(void) {
    __bpf_capsule_yield();
}

// Valid only in Capsule-managed code reached through capsule_call_ctx or
// capsule_continue_ctx. Returns the live borrowed context; cast it to the
// entry's context type. The context is not part of the managed root's
// signature and is never stored in its software frame: the compiler threads
// it as an implicit first argument through context-dependent physical steps.
// The compiler rematerializes this accessor beside each use, so one source
// variable remains convenient across regions. Values derived from the context
// do not survive a suspension point and must be derived and checked again.
...
// continuation. Resume work started with capsule_call_ctx through
// capsule_continue_ctx, supplying the live context of that BPF invocation;
// neither the original pointer nor any value derived from it is saved in the
// fiber. Each continuation is single-use: pass it to exactly one continue or
// reset operation, and use only the newest continuation a fiber has reported.
// The token carries a lease generation, so a duplicate or delayed value is
// rejected even after its fiber has been re-leased to unrelated work. The
// output pointer must have the root's exact return size: a size mismatch
// reports CAPSULE_ERROR_RETURN_MISMATCH and cancels the computation, releasing
// the fiber.
//
...
extern int __bpf_capsule_call(uint32_t fiber, void* context, void* output, uint64_t output_size, uint64_t output_alignment, void* function, ...);
...
extern struct capsule_result __bpf_capsule_continue_ctx(void* context, void* output, uint64_t output_size, uint64_t output_alignment, uint64_t continuation)
    __attribute__((warn_unused_result));
...
#define __bpf_capsule_begin(context, output, output_size, output_alignment, function, ...) \
    __bpf_capsule_result_must_be_used(({ \
        ...
        if (__capsule_fiber != __BPF_CAPSULE_NO_FIBER) { \
            ...
            __capsule_result.status = \
                __bpf_capsule_call(__capsule_fiber, (void*)(context), (void*)(output), (output_size), (output_alignment), (void*)(function), ##__VA_ARGS__); \
...
#define capsule_call(output, function, ...) \
    ({ \
        ...
        __bpf_capsule_begin( \
            (void*)0, __capsule_output, sizeof(*__capsule_output), __BPF_CAPSULE_ALIGNOF(__typeof__(*__capsule_output)), function, ##__VA_ARGS__); \
    })

#define capsule_call_void(function, ...) \
    ({ \
        __BPF_CAPSULE_CHECK_VOID(function, ##__VA_ARGS__); \
        __bpf_capsule_begin((void*)0, (void*)0, 0, 1, function, ##__VA_ARGS__); \
    })

#define capsule_call_ctx(context, output, function, ...) \
    ({ \
        __BPF_CAPSULE_AUTO __capsule_context = (context); \
        __BPF_CAPSULE_AUTO __capsule_output = (output); \
        __BPF_CAPSULE_CHECK_RETURN(__capsule_output, function, ##__VA_ARGS__); \
        __bpf_capsule_begin( \
            __capsule_context, __capsule_output, sizeof(*__capsule_output), __BPF_CAPSULE_ALIGNOF(__typeof__(*__capsule_output)), function, ##__VA_ARGS__); \
    })

#define capsule_call_void_ctx(context, function, ...) \
    ({ \
        __BPF_CAPSULE_AUTO __capsule_context = (context); \
        __BPF_CAPSULE_CHECK_VOID(function, ##__VA_ARGS__); \
        __bpf_capsule_begin(__capsule_context, (void*)0, 0, 1, function, ##__VA_ARGS__); \
    })
...
#define capsule_continue_ctx(context, output, continuation) \
    __bpf_capsule_result_must_be_used(({ \
        __BPF_CAPSULE_AUTO __capsule_context = (context); \
        __BPF_CAPSULE_AUTO __capsule_output = (output); \
        __bpf_capsule_continue_ctx((void*)__capsule_context, (void*)__capsule_output, sizeof(*__capsule_output), \
            __BPF_CAPSULE_ALIGNOF(__typeof__(*__capsule_output)), (continuation)); \
    }))

#define capsule_continue_void_ctx(context, continuation) \
    __bpf_capsule_result_must_be_used(__bpf_capsule_continue_ctx((void*)(context), (void*)0, 0, 1, (continuation)))
